...
- [Pattern 1: Read/write with dynamic approval](#pattern-1-readwrite-with-dynamic-approval)
- [Pattern 2: Background polling with startup](#pattern-2-background-polling-with-startup)
- [Pattern 3: Proactive alerts with bind](#pattern-3-proactive-alerts-with-bind)
...
> **Note:** `startup` runs once at engine init, not per message. The background thread is a daemon — it exits when the main process exits. Use `_lock` when sharing state between the polling thread and `handle`.
...
> **Tip:** `bind` receives the adapter currently active. In multi-adapter mode, it receives the first adapter. If you need to send to Slack specifically, check `type(_adapter).__name__ == "SlackAdapter"` before calling `send`.
