...
def _cleanup_socket(path: str, expected_ino: int) -> None:
    ...
    darken it. Mirrors the platform-side reclaim guard in multi-mithai's Stop."""
    try:
...
def _maybe_start_embedded_api(config: dict, engine, adapter) -> None:
    ...
    multi-mithai's procmgr proxies API requests (including /api/trigger and
    ...
    See multi-mithai docs/agent-uds-contract.md.
    """
