...
:::caution[Caution]
If `mithai doctor` reports an LLM connectivity failure, the agent will start but every message will result in an error. Resolve connectivity issues before deploying.
:::
...
:::tip[Tip]
Add `--verbose` to `ExecStart` during initial deployment to see debug-level logs including skill loading, LLM calls, and tool routing. Remove it once stable to reduce log volume.
:::
...
:::caution[Caution]
Do not mount `config.yaml` with write permissions in the container. Use `:ro` (read-only) to prevent the process from accidentally overwriting your configuration.
:::
...
:::caution[Caution]
If `config.yaml` is committed with a live token in it and pushed to a repository, rotate the token immediately. `${VAR}` references in YAML are the only safe pattern for secrets.
:::
...
:::note[Note]
Memory and state are plain files. `memory/MEMORY.md` is Markdown. Session state files are JSON. You can inspect and edit them directly if needed.
:::
...
:::caution[Caution]
Do not run `mithai run` twice from the same working directory with the same config. Use multi-agent mode or separate working directories with separate state paths.
:::
...
:::note[Note]
Skill upgrades do not affect data in `memory/` or `.mithai/state/`. Your agent's accumulated knowledge is safe across upgrades.
:::
