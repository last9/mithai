...
> **Before you begin**
> - mithai is installed and `mithai chat` works ([Getting started](/getting-started/))
...
- **[Core concepts →](/concepts/)** — Go deeper on skills, Human MCP approval levels, sessions, memory, and adapters.
- **[Skills reference →](/skills-reference/)** — All the hooks a skill can export: `handle`, `resolve_human`, `startup`, `bind`, `MCP_TOOLS`.
- **[Configuration reference →](/configuration/)** — Every option in `config.yaml`.
