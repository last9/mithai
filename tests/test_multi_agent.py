...
from mithai.core.skill_loader import Skill, ToolDefinition, filter_skills, load_skills
...
class TestPerAgentAdapters:
    ...
    def test_create_adapter_with_explicit_config(self):
        ...
        with patch("mithai.adapters.slack.SlackAdapter") as mock_cls:
            ...
            adapter = _create_adapter(config, "slack", adapter_config=per_agent)
...
class TestCreateEnginesMultiIntegration:
    ...
    def test_agent_without_skill_allowlist_gets_all_skills(self, tmp_path):
        ...
        # all_access agent has both skills
        assert set(engines["all_access"]._skills.keys()) == {"shell", "memory"}

        # limited agent has only shell
        assert set(engines["limited"]._skills.keys()) == {"shell"}
